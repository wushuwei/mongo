...
func GetPatternRecognition(ticker string) {
	finnhubClient := finnhub.NewAPIClient(finnhub.NewConfiguration()).DefaultApi
	auth := context.WithValue(context.Background(), finnhub.ContextAPIKey, finnhub.APIKey{
		Key: "c17bkh748v6se55vr9eg", // Replace this
	})
	// Pattern recognition
	patterns, _, err := finnhubClient.PatternRecognition(auth, ticker, "D")
	if err != nil {
		fmt.Println(err)
	} else {
		// fmt.Printf("%+v\n", patterns)
		for _, v := range patterns.Points {

			fmt.Println("Symbol:", v["symbol"], " patternname: ", v["patternname"], " patterntype: ", v["patterntype"], " time: ", v["atime"])
			// fmt.Printf("%+v\n", v)
		}
	}
